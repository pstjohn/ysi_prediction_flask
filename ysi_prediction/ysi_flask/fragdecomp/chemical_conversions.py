...
def get_smiles_from_cas(cas, db="nih"):
    ...
    try:
        if db == "nih":
            smiles = (
                urlopen(
                    "http://cactus.nci.nih.gov/chemical/structure/{}/SMILES".format(
                        cas
                    ),
                    context=context,
                )
            ...
            html = (
                urlopen(
                    "http://webbook.nist.gov/cgi/cbook.cgi?ID={}&Units=SI".format(cas),
                    context=context,
                )
                .read()
                .decode()
            ...
            html = (
                urlopen(
                    "http://chem.sis.nlm.nih.gov/chemidplus/rn/{}".format(cas),
                    context=context,
                )
                .read()
                .decode()
        ...
        cas_sum = (
            sum(
                list(
                    (
                        int(num) * (i + 1)
                        for i, num in enumerate(cas[:-2].replace("-", "")[::-1])
                    )
                )
            )
            % 10
...
def get_cas_from_inchi(inchi):
    try:
        html = (
            urlopen(
                "http://webbook.nist.gov/cgi/cbook.cgi?InChI={}&Units=SI".format(inchi)
            )
            .read()
            .decode()
        )
...
def get_cas_from_name(name, db="webbook"):
    try:
        if db == "webbook":
            html = (
                urlopen(
                    "http://webbook.nist.gov/cgi/cbook.cgi?Name={}&Units=SI&cTP=on".format(
                        name.lower().replace(",", "%2C")
                    )
                )
                .read()
                .decode()
